...
//The first element is a pointer which refers to a pattern testing function
//The second element is the associated score of this pattern 
...
{
	//coordinates of the tile compared with the tile at (x,y)
	int new_x = 0;
	int new_y = 0;

	//testing if the tile at (x,y) is monochrome so it can be the center of a pattern
	struct color* c = is_monochrome(tile(b,x,y));
	if (c == NULL)
		return 0; 
	else {
		//testing the correspondance between edges of the (x,y) tile and the tile in direction d1
		...
		//testing the correspondance between edges of the (x,y) tile and the tile in direction d2
